...
function AbaFavoritos() {
  createServer({
    routes() {
      this.get("/api/products/favoritos", () => ({
        products: [
          {
            id: 1,
          ...
          <Item title="Clique para favoritar">
            {products.map((product) => (
              <ItemContainer key={product.id}>
                ...
                {/* <TitlePane>Kit 10 Un. Adesivo 3m Porta Cartão De Silicone Para Celular{product.name}</TitlePane>
                <PricePane>R$ 31.67{product.price}</PricePane>
                <SalesPane>Total de 4.750{product.sales}</SalesPane>
                <SalesDescriptionPane>150 vendas{product.salesDescription}</SalesDescriptionPane>
                <StockPane>0 unid{product.stock}</StockPane> */}
                <Favoritar />
              </ItemContainer>
            ))}
          </Item>
